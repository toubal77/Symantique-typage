...
class Formatter {
  ...
  def format(typ: Typ): String = typ match {
    case BoolType => "Bool"
    case IntType => "Int"
    case Arrow(fromType, toType) => {
      val formattedFrom = format(fromType)
      val formattedTo = format(toType)
      if (fromType.isInstanceOf[Arrow])
        s"($formattedFrom) -> $formattedTo"
      else
        s"$formattedFrom -> $formattedTo"
    }
    case Nat => "Nat"
    case Lambda(paramType, returnType) => {
      val formattedParam = format(paramType)
      val formattedReturn = format(returnType)
      s"($formattedParam -> $formattedReturn)"
    }
  }
}
