...
class ParserTest extends AnyFunSuite with Matchers {
  ...
  test(""""parse(recordTyp, "{}") --> Record()""") {
    val result = parse(recordTyp, "{}")
    result should matchPattern {case Success(Record(_), _) => }
    inside(result) { case Success(typ, _) =>
        assert(typ === Record(ListMap())) } 
  }
  test(""""parse(recordTyp, "{lab1 : Bool}") --> Record(lab1 -> Bool)""") {
    val result = parse(recordTyp, "{lab1 : Bool}")
    result should matchPattern {case Success(Record(_), _) => }
    inside(result) { case Success(typ, _) =>
        assert(typ === Record(ListMap(l1 -> Bool))) } 
  }
  test(""""parse(recordTyp, "{lab1 : Bool, lab2 : Nat}")"
      + "--> Record(lab1 -> Bool, lab2 -> Nat)""") {
    val result = parse(recordTyp, "{lab1 : Bool, lab2 : Nat}")
    result should matchPattern {case Success(Record(_), _) => }
    inside(result) { case Success(typ, _) =>
        assert(typ === Record(ListMap(l1 -> Bool, l2 -> Nat))) } 
  }
  test(""""parse(recordTyp, "{lab1 : {lab2 : Bool}}")"
      + "--> Record(lab1 -> Record(lab2 -> Bool)""") {
    val result = parse(recordTyp, "{lab1 : {lab2 : Bool}}")
    result should matchPattern {case Success(Record(_), _) => }
    inside(result) { case Success(typ, _) =>
        assert(typ === Record(ListMap(l1 -> Record(ListMap(l2 -> Bool))))) } 
  }
  ...
  test(""""parse(atomTyp, "{lab1 : Bool, lab2 : Nat}")"
      + "--> Record(lab1 -> Bool, lab2 -> Nat)""") {
    val result = parse(atomTyp, "{lab1 : Bool, lab2 : Nat}")
    result should matchPattern {case Success(Record(_), _) => }
    inside(result) { case Success(typ, _) =>
        assert(typ === Record(ListMap(l1 -> Bool, l2 -> Nat))) } 
  }
  ...
  test(""""parse(fields, "") --> Failure(...)""") {
    val result = parse(fields, "")
    result should matchPattern {case Failure(_, _) => }
  }
  test(""""parse(fields, "lab1 = true") --> ListMap(lab1 -> True)""") {
    val result = parse(fields, "lab1 = true")
    result should matchPattern {case Success(_, _) => }
    inside(result) { case Success(fields, _) =>
        assert(fields === ListMap(l1 -> True)) } 
  }
  test(""""parse(fields, "lab1 = false, lab2 = 0")"
      + "--> ListMap(lab1 -> False, lab2 -> Zero)""") {
    val result = parse(fields, "lab1 = false, lab2 = 0")
    result should matchPattern {case Success(_, _) => }
    inside(result) { case Success(fields, _) =>
        assert(fields === ListMap(l1 -> False, l2 -> Zero)) } 
  }
  test(""""parse(fields, "lab1 = {lab2 = true}")"
      + "--> ListMap(lab1 -> Rec(lab2 = True)""") {
    val result = parse(fields, "lab1 = {lab2 = true}")
    result should matchPattern {case Success(_, _) => }
    inside(result) { case Success(fields, _) =>
        assert(fields === ListMap(l1 -> Rec(ListMap(l2 -> True)))) } 
  }
  
  
  test(""""parse(record, "{}") --> Rec()""") {
    val result = parse(record, "{}")
    result should matchPattern {case Success(Rec(_), _) => }
    inside(result) { case Success(term, _) => assert(term === Rec(ListMap())) }
  }
  test(""""parse(record, "{lab1 = false, lab2 = 0}")"
      + "--> Rec(lab1 -> False, lab2 -> Zero)""") {
    val result = parse(record, "{lab1 = false, lab2 = 0}")
    result should matchPattern {case Success(_, _) => }
    inside(result) { case Success(term, _) =>
        assert(term === Rec(ListMap(l1 -> False, l2 -> Zero))) } 
  }
  ...
  test(""""parse(atom, "{lab1 = false, lab2 = 0}")"
      + "--> Rec(lab1 -> False, lab2 -> Zero)""") {
    val result = parse(atom, "{lab1 = false, lab2 = 0}")
    result should matchPattern {case Success(_, _) => }
    inside(result) { case Success(term, _) =>
        assert(term === Rec(ListMap(l1 -> False, l2 -> Zero))) } 
  }
  ...
  test(""""parse(lambda, "lambda x: Nat. t") --> Abs(x, Nat, t)""") {
    parse(lambda, "lambda x: Nat. t") should matchPattern
  ...
  test(""""parse(open, "if true then 0 else 1")"
      + "--> Cond(True, Zero, Succ(Zero))""") {
