...
class UtilTest extends AnyFunSuite with Matchers {
  ...
  test("{l1 = t1, l2 = t2}[t/x] --> {l1 = t1, l2 = t2}") {
    val init = Rec(ListMap((l1, t1), (l2, t2)))
    assert(subst(x, t, init) === init)
  }
  test("{l1 = 0, l2 = x}[t/x] --> {l1 = 0, l2 = t}") {
    val init = Rec(ListMap((l1, Zero), (l2, x)))
    val result = Rec(ListMap((l1, Zero), (l2, t)))
    assert(subst(x, t, init) === result)
  }
  test("{l1 = {l2 = 0}}[t/x] --> {l1 = {l2 = 0}}") {
    val init = Rec(ListMap((l1, Rec(ListMap((l2, Zero))))))
    assert(subst(x, t, init) === init)
  }
  test("{l1 = {l2 = x}}[t/x] --> {l1 = {l2 = t}}") {
    val init = Rec(ListMap((l1, Rec(ListMap((l2, x))))))
    val result = Rec(ListMap((l1, Rec(ListMap((l2, t))))))
    assert(subst(x, t, init) === result)
  }
  test("({l1 = t1, l2 = t2}.l2)[t/x] --> {l1 = t1, l2 = t2}.l2") {
    val init = Proj(Rec(ListMap((l1, t1), (l2, t2))), l2)
    assert(subst(x, t, init) === init)
  }
  test("({l1 = t1, l2 = x}.l2)[t/x] --> {l1 = t1, l2 = t}.l2") {
    val init = Proj(Rec(ListMap((l1, t1), (l2, x))), l2)
    val result = Proj(Rec(ListMap((l1, t1), (l2, t))), l2)
    assert(subst(x, t, init) === result)
  }
  ...
  test("{l1 = 0, l2 = lambda x : Bool. t, l3 = {l1 = 1}} est une valeur") {
    val rec1 = Rec(ListMap(l1 -> Succ(Zero)))
    assert(isVal(rec1))
    val rec2 = Rec(ListMap(l1 -> Zero, l2 -> Abs(x, Bool, t), l3 -> rec1))
    assert(isVal(rec2))
  }
  ...
  test("{l1 = pred 0} n'est pas une valeur") {
    assert(!isVal(Rec(ListMap(l1 -> Pred(Zero)))))
  }
  test("{l1 = {l2 = pred 0}} n'est pas une valeur") {
    assert(!isVal(Rec(ListMap(l1 -> Rec(ListMap(l2 -> Pred(Zero)))))))
  }
  ...
  test("{l1 = x} n'est pas clos") {
    assert(!isClosed(Rec(ListMap(l1 -> x))))
  }  
  test("{l1 = lambda y : Bool. x} n'est pas clos") {
    assert(!isClosed(Rec(ListMap(l1 -> Abs(y, Bool, x)))))
  }  
  test("{l1 = y}.l1 n'est pas clos") {
    assert(!isClosed(Proj(Rec(ListMap(l1 -> y)), l1)))
  }  
  test("{l1 = lambda x : Bool. y}.l1 n'est pas clos") {
    assert(!isClosed(Proj(Rec(ListMap(l1 -> Abs(x, Bool, y))), l1)))
  }  
}