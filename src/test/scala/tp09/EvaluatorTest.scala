...
class EvaluatorTest extends AnyFunSuite with PrivateMethodTester {
  ...
  test("let x = true in x --> true") {
    val init = LetIn(x, True, x)
    val result = True
    assert((evaluator invokePrivate eval(init)) === result)
  }
  test("let x = 0 in (id x) --> id 0") {
    val init = LetIn(x, Zero, App(id, x))
    val result = App(id, Zero)
    assert((evaluator invokePrivate eval(init)) === result)
  }
  test("let x = (id False) in x --> let x = False in x") {
    val init = LetIn(x, App(id, False), x)
    val result = LetIn(x, False, x)
    assert((evaluator invokePrivate eval(init)) === result)
  }
  ...
  test("Fix(lambda x : Nat -> Bool. x 0) --> "
      + "Fix(lambda x : Nat -> Bool. x 0) 0") {
    val init = Fix(Abs(x, Fct(Nat, Bool), App(x, Zero)))
    val result = App(Fix(Abs(x, Fct(Nat, Bool), App(x, Zero))), Zero)
    assert((evaluator invokePrivate eval(init)) === result)
  }
  test("Fix(id id) --> Fix(id)") {
    val init = Fix(App(id, id))
    val result = Fix(id)
    assert((evaluator invokePrivate eval(init)) === result)
  }
  
  
  test("{lab1 = 0, lab2 = pred succ 0, lab3 = if true then false else true}"
      + "--> {lab1 = 0, lab2 = 0, lab3 = if true then false else true}") {
    val init = Rec(ListMap(lab1 -> Zero, lab2 -> Pred(Succ(Zero))
        , lab3 -> Cond(True, False, True)))
    val result = Rec(ListMap(lab1 -> Zero, lab2 -> Zero
        , lab3 -> Cond(True, False, True)))
    assert((evaluator invokePrivate eval(init)) === result)
  }
  test("{lab1 = 0, lab2 = pred succ 0, lab3 = if true then false else true}"
      + "-/-> {lab1 = 0, lab2 = pred succ 0, lab3 = false}") {
    val init = Rec(ListMap(lab1 -> Zero, lab2 -> Pred(Succ(Zero))
        , lab3 -> Cond(True, False, True)))
    val result = Rec(ListMap(lab1 -> Zero, lab2 -> Pred(Succ(Zero))
        , lab3 -> False))
    assert((evaluator invokePrivate eval(init)) !== result)
  }
  
  
  test("{lab1 = 0, lab2 = 1, lab3 = true}.lab2 --> 1") {
    val init = Proj(
        Rec(ListMap(lab1 -> Zero, lab2 -> Succ(Zero), lab3 -> True))
        , lab2)
    val result = Succ(Zero)
    assert((evaluator invokePrivate eval(init)) === result)
  }
  test("{lab1 = 0, lab2 = 1, lab3 = id true}.lab2"
      + "--> {lab1 = 0, lab2 = 1, lab3 = true}.lab2") {
    val init = Proj(
        Rec(ListMap(lab1 -> Zero, lab2 -> Succ(Zero), lab3 -> App(id, True)))
        , lab2)
    val result = Proj(
        Rec(ListMap(lab1 -> Zero, lab2 -> Succ(Zero), lab3 -> True))
        , lab2)
